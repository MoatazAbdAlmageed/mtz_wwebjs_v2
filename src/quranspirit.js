...
const pm2_filename = "index";
const https = require('https');
const http = require('http');
const DomainName = "test.quranspirit.com";
...
const corsOptions = {
 origin: [
    "https://test.quranspirit.com",
    "http://198.54.114.214",
    "https://198.54.114.214",
    "http://198.54.114.86",
    "https://198.54.114.86l",
	 "*,",
	     "https://system.quranspirit.com",
	 "https://system.quranmadrasah.com"
  ],
...
app.post("/get-group-id2", async (req, res) => {
  ...
  try {
    // Get all chats and find the target group by name
    const chats = await client.getChats();
    const targetName = groupName.trim().toLowerCase();

    const group = chats.find(chat =>
      chat.isGroup && chat.name.trim().toLowerCase() === targetName
    );

    if (!group) {
      console.log(`Group not found for name: "${groupName}"`);
      return res.status(404).json({ status: 404, error: "Group Not Found" });
    }

    console.log(`Found group: ${group.name}`);

    // Normalize parentPhone: remove non-digits, convert leading zero to country code 20
    ...
    // Get client own number
    const clientInfo = await client.info;
    const myNumber = clientInfo.wid._serialized;
    console.log("Client's own number:", myNumber);

    // Fetch full group metadata with participants
    const groupMetadata = await client.getChatById(group.id._serialized);
    const participants = groupMetadata.participants || [];

    // Log all participants
    console.log(`Group "${group.name}" members (${participants.length}):`);
    participants.forEach((p, i) => {
      const name = p.notifyName || p.name || "Unknown";
      console.log(`${i + 1}. ${name} — ${p.id._serialized}`);
    });

    // Check if normalizedPhone is in participants with @c.us ID
    const normalizedPhoneNumber = onlyDigits; // e.g. "201150064746"
    const isInGroup = participants.some(p => {
      const id = p.id._serialized;
      if (id.endsWith("@c.us")) {
        return id.startsWith(normalizedPhoneNumber);
      }
      return false;
    });

    // Also allow if the normalized phone matches client's own number
