...
app.post("/send-to-all", async (req, res) => {
  ...
  if (chats.length > 0) {
    chats.map(async chat => {
      ...
      if (req.body.sendToContacts && !chat.isGroup) {
        await client.sendMessage(chat.id._serialized, `${req.body.message}  via whatsapp bot`)
      }
      if (req.body.sendToGroups && chat.isGroup) {
        await client.sendMessage(chat.id._serialized, `${req.body.message}  via whatsapp bot`)
      }
    });
